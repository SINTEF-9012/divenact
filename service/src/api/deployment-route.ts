...
import { getDeployment, listDeployments, clearDeployments, queryAppliedDevices, queryTargetedDevices } from "../deployment";
...
router.get('/:deployment/targeted', async (req, res) => {    
    res.json(await queryTargetedDevices(req.params['deployment']));
});
